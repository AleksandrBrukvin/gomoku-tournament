...
public class GomokuGame implements Game{
	 ...
	 *
	 */
	public static class PlayerScoreValue implements Result{
		public int wins = 0;
		public int losses = 0;
		public int draws = 0;
	 ...
	 *
	 */
	public static class PlayerScoreConf {
		/**
		 * Value ("color") of the player
		 * who's score it is.
		 */
		public SquareState playerValue;
		/**
		 * Opponent player.
		 */
	...
	public static class PlayerScore {
		public HashMap<PlayerScoreConf, PlayerScoreValue> scoreValues = new HashMap<PlayerScoreConf, PlayerScoreValue>();
	...
	public static class PlayerScores {
		...
		public static ObservableList<GameStats> getGameStats(Strategy strategy) {
			Player player = GomokuTournamentController.getPlayerbyName(strategy);
			PlayerScore ps = scores.get(player);
			if (ps == null) {
				return null;
			}
			
			ObservableList<GameStats> games = FXCollections.observableArrayList();
			...
			for (PlayerScoreConf key : ps.scoreValues.keySet()) {
				...
				if (psv.wins == 1) {
					gameResult = "Win";
				}else if (psv.losses == 1) {
					gameResult = "Lose";
				}else if (psv.draws == 1) {
					gameResult = "Draw";
				}
				if(key.playerValue.name().equals("WHITE")){
					whitePlayerName = (String) key.opponent.getName();
					blackPlayerName = player.getName();
				}else{
					whitePlayerName = player.getName();
					blackPlayerName = (String) key.opponent.getName();
				}
				games.add(new GameStats(whitePlayerName,blackPlayerName,(Integer)key.boardSize,gameResult));
			}
			return games;
		}
		
		public static PlayerStats getPlayerStats(Strategy strategy) {
			Player player = GomokuTournamentController.getPlayerbyName(strategy);
			PlayerScore ps = scores.get(player);
			if (ps == null) {
				return null;
			...
			ArrayList<String> gamesLog = new ArrayList<String>();
			
			for (PlayerScoreConf key : ps.scoreValues.keySet()) {
				PlayerScoreValue psv = ps.scoreValues.get(key);
				wins = wins + psv.wins;
				draws = draws + psv.draws;
				losses = losses + psv.losses;
				winRate = winRate + psv.getPointsPercentage();
				String gameLog = " " + key.playerValue+ " | " + key.opponent 
						+" | board: "+ key.boardSize + " | " + psv.wins + "-" + psv.draws 
						+ "-" + psv.losses + " / " + psv.getGameCount() + " |";
				gamesLog.add(gameLog);
			}
			winRate = winRate / ps.scoreValues.keySet().size();
			
			return new PlayerStats(player.getName(), strategy.toString(), wins, draws, losses, (int) winRate, gamesLog);
		}
	}
	...
	public GomokuGame(Player playerWhite, Player playerBlack, int height, int width) {
		this(playerWhite, playerBlack, height, width, null);
	}
	
	public GomokuGame(Player playerWhite, Player playerBlack, int height, int width, SimpleBoard initialBoard) {
	...
	/**
	 * @return current game status property
	 */
	...
	/**
	 * @return current player status property
	 */
	...
	/**
	 * @param filename History log file name
	 */
	...
	/**
	 * @param inReplayMode sets game into square mode
	 */
	 ...
	 */
	@Override
	public Result play(Strategy... strategies) throws Exception {
		GomokuTournamentController controller = new GomokuTournamentController(this);
		
		controller.newTournament(10, strategies[0], strategies[1]);
		controller.newTournament(20, strategies[0], strategies[1]);
		
		return new PlayerScore(playerBlack).scoreValues.get(strategies[0]);
	}

	/**
	 * @return player move answer
	 */
	...
	/**
	 * @param playerAnswer player move answer
	 */
